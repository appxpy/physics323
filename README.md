...
Барометрическая формула определяет зависимость давления или плотности газа от высоты в поле силы тяжести \cite{1}. Эта зависимость применяется в стационарных условиях для идеального газа, имеющего постоянную температуру \cite{2}.

Барометрическая формула Лапласа дает зависимость давления от высоты и имеет следующий вид: $p = p_0 \cdot \exp{(-\frac{\mu g h}{R T})}$, где $\mu$ - молярная масса газа, $h$ - высота, $T$ - температура, $p_0$ - давление у поверхности Земли, $R = 8,31 \times 10^3 ,\text{Дж/кмоль} \cdot \text{К}$ - универсальная газовая постоянная \cite{4}.
